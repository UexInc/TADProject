...
public class EntryPanel extends JFrame implements ActionListener {
	...
	private void renderComponents(String[] texts, byte option) {
		...
		else if (option == 4) renderTree(texts, action);
	...
	private void renderTree(String[] texts, int action) {
		@SuppressWarnings("unchecked")
		LinkedTree<Object> tree = ((LinkedTree<Object>) Types.type);		
		String[] options = new String[tree.size()];
		int c = 0;
		for (Position<Object> pos : tree.positions()) {
			System.out.println(pos.element());
			options[c] = pos.element().toString();
			c++;
		}
